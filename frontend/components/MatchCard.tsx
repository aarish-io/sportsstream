...
export const MatchCard: React.FC<MatchCardProps> = ({ match, isActive, onWatch, onUnwatch }) => {
  ...
  return (
    <div className={`
      relative p-5 rounded-2xl border-2 border-black bg-white/95 transition-all duration-200 backdrop-blur-sm
      ${isActive ? 'shadow-hard translate-x-[-2px] translate-y-[-2px] ring-2 ring-brand-yellow ring-offset-2' : 'hover:shadow-hard-sm'}
    `}>
      <div className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-brand-yellow via-amber-200 to-brand-blue rounded-t-2xl" />
      {/* Header: Sport & Status */}
